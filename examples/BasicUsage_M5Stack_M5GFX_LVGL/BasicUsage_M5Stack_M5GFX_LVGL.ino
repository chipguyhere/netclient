/*
 * BasicUsage_M5Stack_M5GFX_LVGL.ino - Example sketch for chipguy_MQTT_netclient library
 *
 ...
 *
 * ALSO KNOWN ISSUE -- M5GFX will conflict with W5500 Ethernet if they share the
 * same SPI pins (Core, Core2, StamPLC).  Not a problem but you must use the SPI
 * NetClient example instead for these devices (and not M5GFX).  The SPI example uses
 * a built-in LVGL driver for these three products that properly share the SPI bus with
 * ESP32's Ethernet support.
 *
...
#if defined(CHIPGUY_NETCLIENT_ETHERNET_SHARES_DISPLAY_SPI)
// M5GFX drives the display via direct SPI-register access without
// honoring bus arbitration, so it cannot safely coexist with an
// Ethernet adapter wired onto the same SPI bus. The
// BasicUsage_M5Stack_SPI_LVGL example drives the display through
// ESP-IDF-synchronized SPI primitives instead and is safe on a
// shared bus.
#error "M5GFX library uses direct SPI register access and cannot share its bus with the configured Ethernet adapter. Use BasicUsage_M5Stack_SPI_LVGL example instead."
#endif
...
void setup() {
    ...
    Serial.println("\n\nchipguy_MQTT_netclient M5Stack M5GFX LVGL Example");
    Serial.println("=============================================\n");
...
void loop() {
    ...
    // UI logic.
    // LVGL's timer handler goes here, which pushes frames to the display when needed.
    // Here is where anything else that keeps any screen updated user interface
    // running should go as well.
    lv_timer_handler();
