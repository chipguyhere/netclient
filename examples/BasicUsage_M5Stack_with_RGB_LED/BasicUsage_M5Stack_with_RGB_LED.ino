/*
 * BasicUsage_M5Stack_with_RGB_LED.ino - Example sketch for chipguy_MQTT_netclient library
 *
 ...
 * - Using M5Unified to detect and drive an RGB status LED (WS2812)
 *
 * In Arduino IDE, select the specific M5Stack product you are using as the board type.
 * This is required in order to auto-detect which pin the status LED connects to.
 *
...
// M5Unified and M5.* is referenced only to look up the pin number of an RGB status LED.
// If you know the pin number, you may optionally remove all M5 references and hardcode it.
#include <M5Unified.h>
...
// RGB LED pin detected from M5Unified (-1 = no LED on this board)
int8_t rgbLedPin = -1;

// Set the RGB status LED (no-op if the board has no LED)
void setStatusLED(uint8_t r, uint8_t g, uint8_t b) {
    if (rgbLedPin < 0) return;
    // neopixelWrite() is built into the Arduino ESP32 core and handles
    // WS2812 GRB ordering and RMT timing internally.
    neopixelWrite(rgbLedPin, r, g, b);
}

void setup() {
    Serial.begin(115200);

    // Initialize M5Unified (auto-detects the board from the Arduino IDE
    // board selection).  This must happen before getPin() is called.
    auto cfg = M5.config();
    M5.begin(cfg);

    // Detect whether this board has a WS2812 RGB LED ("neopixel")
    rgbLedPin = M5.getPin(m5::pin_name_t::rgb_led);
    if (rgbLedPin >= 0) {
        Serial.printf("[LED] RGB LED found on pin %d\n", rgbLedPin);
        setStatusLED(0, 0, 0);  // start dark
    } else {
        Serial.println("[LED] No RGB LED on this board");
    }

    Serial.println("\n\nchipguy_MQTT_netclient M5Stack RGB LED Example");
    Serial.println("================================================\n");
...
void loop() {
    ...
    while (NetClient.eventAvailable()) {
        ...
        switch (client_event.type) {
            ...
            // Status color reflects connection progress.  On boards with a
            // WS2812 RGB LED (detected at startup via M5Unified), the LED is
            // set to the corresponding color.  Boards without an RGB LED
            // still log the color to Serial.
            case EventType::STATUS_COLOR_CHANGE: {
                ...
                switch (event.color) {
                    case StatusColor::CYAN:
                        setStatusLED(0, 20, 20);     // initializing
                        break;
                    case StatusColor::RED:
                        setStatusLED(20, 0, 0);      // no network connection (yet)
                        break;
                    case StatusColor::YELLOW:
                        setStatusLED(20, 20, 0);     // network up, MQTT connecting
                        break;
                    case StatusColor::GREEN:
                        setStatusLED(0, 20, 0);      // MQTT connected
                        break;
                }
                break;
            }
